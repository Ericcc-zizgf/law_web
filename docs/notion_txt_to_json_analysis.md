...
`txt_to_json.py` 不再讓 7B 模型重新抄寫整份法律文件，而是使用 Python 從 TXT 中擷取固定欄位與段落。模型只需要在後續 RAG 階段協助理解內容，降低長文輸出截斷、欄位錯置與委員會名單污染的問題。
...
目前「理由」仍是一個完整字串。若要提升 RAG 精準度，下一步可依「一、」「二、」「三、」切成陣列，讓每個法律爭點成為獨立 chunk。
